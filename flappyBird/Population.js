class Population {
  ...
  setBestPlayer() {
    ...
    if (tempBest.score >= this.bestScore) {
      ...
      // Save best player to localStorage
      this.saveBestPlayer();
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------------
  // Save the best player's brain to localStorage
  saveBestPlayer() {
    if (this.bestPlayer) {
      var saveData = {
        score: this.bestScore,
        gen: this.gen,
        brain: this.serializeGenome(this.bestPlayer.brain)
      };
      localStorage.setItem('flappyBirdBestAI', JSON.stringify(saveData));
      console.log("AI saved! Score: " + this.bestScore + " Gen: " + this.gen);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------------
  // Export the best player to a downloadable JSON file
  exportBestPlayer() {
    if (this.bestPlayer) {
      var saveData = {
        score: this.bestScore,
      ...
      link.href = url;
      link.download = 'flappybird_ai_score_' + this.bestScore + '_gen_' + this.gen + '.json';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      
      console.log("AI exported to file! Score: " + this.bestScore + " Gen: " + this.gen);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------------
  // Import player from a JSON file
  importPlayerFromFile(fileContent) {
    try {
      var data = JSON.parse(fileContent);
      console.log("Importing AI with score: " + data.score + " from gen: " + data.gen);
      this.bestScore = data.score;
      this.bestPlayer = new Player();
      this.bestPlayer.brain = this.deserializeGenome(data.brain);
      this.bestPlayer.brain.generateNetwork();
      this.bestPlayer.score = data.score;
      this.bestPlayer.gen = data.gen;
      
      // Also save to localStorage
      localStorage.setItem('flappyBirdBestAI', JSON.stringify(data));
      
      console.log("AI successfully imported!");
      return true;
    } catch (e) {
      console.error("Failed to import AI:", e);
      return false;
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------------
  // Load the best player from localStorage
  loadBestPlayer() {
    var savedData = localStorage.getItem('flappyBirdBestAI');
    if (savedData) {
      var data = JSON.parse(savedData);
      console.log("Loaded AI with score: " + data.score + " from gen: " + data.gen);
      this.bestScore = data.score;
      this.bestPlayer = new Player();
      this.bestPlayer.brain = this.deserializeGenome(data.brain);
      this.bestPlayer.brain.generateNetwork();
  ...
  //------------------------------------------------------------------------------------------------------------------------------------------------
  // Serialize genome to JSON-compatible object
  serializeGenome(genome) {
    return {
      genes: genome.genes.map(g => ({
        fromNode: g.fromNode.number,
        toNode: g.toNode.number,
        weight: g.weight,
        enabled: g.enabled,
        innovationNo: g.innovationNo
      })),
      nodes: genome.nodes.map(n => ({
        number: n.number,
        layer: n.layer
      })),
      ...
      nextNode: genome.nextNode,
      biasNode: genome.biasNode
    };
  }

  //------------------------------------------------------------------------------------------------------------------------------------------------
  // Deserialize genome from JSON object
  deserializeGenome(data) {
    var genome = new Genome(data.inputs, data.outputs, true);
    genome.layers = data.layers;
    genome.nextNode = data.nextNode;
    genome.biasNode = data.biasNode;
    
    // Recreate nodes
    genome.nodes = [];
    for (var i = 0; i < data.nodes.length; i++) {
      var node = new Node(data.nodes[i].number);
      node.layer = data.nodes[i].layer;
    ...
    for (var i = 0; i < data.genes.length; i++) {
      var gData = data.genes[i];
      var fromNode = genome.getNode(gData.fromNode);
      var toNode = genome.getNode(gData.toNode);
      var gene = new connectionGene(fromNode, toNode, gData.weight, gData.innovationNo);
      gene.enabled = gData.enabled;
